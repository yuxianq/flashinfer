...
def test_attention_ts_softmax_stats_trace_outputs(
    ...
    if kind == "decode":
        ...
        if use_wrapper:
            ...
            target = prims_ts_batch_mla_decode_with_kv_cache
            kwargs.update(max_seq_len_q=max_q, kv_lora_rank=512, qk_rope_head_dim=64)
    if not use_wrapper:
        kwargs["mask_type"] = "causal"
        if kind == "mla" and entrypoint == "one_shot":
            kwargs["max_kv_len"] = 64
        if entrypoint == "standalone":
            kwargs.update(
                max_seq_len=64, workspace_buffer=torch.empty(128, dtype=torch.uint8)
            )
        if store_softmax_stats:
...
class _PagedCase:
    ...
    def standalone(self, stats):
        ...
        if self.kind == "decode":
            size = get_prims_ts_batch_decode_workspace_size(
                **common,
                num_qo_heads=self.q.shape[-2],
                num_kv_heads=self.k.shape[1],
                head_dim=self.q.shape[-1],
            ...
            size = get_prims_ts_batch_mla_decode_workspace_size(
                **common,
                num_heads=self.q.shape[-2],
                kv_lora_rank=512,
                qk_rope_head_dim=64,
            )
        workspace = torch.zeros(size, device=self.q.device, dtype=torch.uint8)
        common = dict(
            query=self.q,
        ...
        if self.kind == "decode":
            return prims_ts_batch_decode_with_kv_cache(
                **common,
                kv_cache=(self.k, self.v),
                seq_len_q=self.max_q,
                window_left=self.window_left,
            )
        return prims_ts_batch_mla_decode_with_kv_cache(
            **common,
            kv_cache=self.k,
            kv_lora_rank=512,
            qk_rope_head_dim=64,
...
def _make_paged_case(
    ...
    *,
    ...
    v = k[..., :512] if mla else (0.2 * torch.randn_like(k.float())).to(dtype)
...
def test_attention_ts_paged_softmax_stats(kind, kwargs):
    ...
    if kind != "prefill":
        standalone_stats = torch.full_like(stats, torch.nan)
        standalone_output = case.standalone(standalone_stats)
        _assert_result(standalone_output, standalone_stats, case)
...
def test_attention_ts_decode_softmax_stats_forced_reduction(
    ...
    case = _make_paged_case(
        "decode",
        heads=64 if family == "keeps" else 16,
        packed=False,
        kv_len=4097,
        max_q=1,
        mask_type="dense",
    )
    # The pinned public API limits GQA to 32; its ungrouped Keeps KV128
    # configuration needs GQA64. Grouped Keeps KV256 covers the same reducer
    # families with the supported 64-query/2-KV-head geometry.
    compile_cache = _force_decode_topology(
        monkeypatch, case, family, reduction, tile_kv=256 if family == "keeps" else 128
    )
