...
def _check_large_logits(
    ...
    *,
    ...
    # One KV iteration per split keeps the exact 512-key/two-split regression.
    monkeypatch.setattr(fmha_decode_config, "MIN_LOOP_ITERS_PER_SPLIT", 1)
    ...
    # The dev wrapper supports Hq/Hkv <= 32. Its qualified paged BF16 Keeps
    # profile is grouped Q64/KV256, with two 256-token instances per split.
    kv_heads = 2 if keeps else 1
    tile_kv = 256 if keeps else 128
    kv_len = max(512, splits * 2 * tile_kv)
    page_size = 32
    modes = {
        "direct": "disabled",
    ...
    assert config.splits_kv == config.max_splits_kv == splits
    assert kv_len // (config.tile_size_kv * config.num_insts_kv) >= splits
    assert config.use_keeps_mma_ab == keeps
    assert not keeps or config.supports_grouped_keeps
    ...
    try:
        ...
        key = torch.zeros(
            (kv_len // page_size, kv_heads, page_size, 128),
